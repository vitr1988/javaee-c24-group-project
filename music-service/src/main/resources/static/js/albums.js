...
function createTracksSelect(selector) {
    return new TomSelect(selector, {
        preload: true,
/*        load: function (query, callback) {
            fetch("/api/artists/")
...
const selectTracks = createTracksSelect('#edit-track-select');
const errorTracks = $('#trackErrorLabel');
...
const albumKeyInputErrorLabelMap = new Map()
    ...
    .set("tracks", [[selectTracks], [() => selectTracks.getValue()], [setTracksByArtist, wrapToJsonWithId], errorTracks, true]);

editAlbumModal.on('show.bs.modal', function (event) {

    if (event.target.id === editAlbumModal.attr('id')) {

        resetFormData(albumKeyInputErrorLabelMap);

        const albumId = $(event.relatedTarget).data('album-id');

        const isAddForm = !(albumId);

        console.log(isAddForm);

        if (isAddForm) {
            albumReleaseDatapicker.datepicker("update", '');
        } else {
            setFormDataFromResponse('/api/albums/' + albumId, albumKeyInputErrorLabelMap);
        }
        ...
        editAlbumForm.off('submit').on('submit', function (event) {
            ...
            $.ajax({
                type: isAddForm ? "POST" : "PUT",
                url: isAddForm ? "/api/albums" : "/api/albums/" + albumId,
                data: JSON.stringify(serializeFormData(albumKeyInputErrorLabelMap, isAddForm)),
                contentType: 'application/json',
                success: function (response) {
                    editAlbumModal.modal('hide');
                    showSuccessAlert("Album edited!")
                    window.location.reload();
                },
                error: function (xhr, status, error) {
                    if (xhr.responseJSON && xhr.responseJSON.code === "VALIDATION_FAILED") {
...
function setTracksByArtist(inputs, response){
    console.log(response);
    inputs[0].settings.load = function(query, callback) {
        fetchTracksByArtist(response.artist.id)(query, callback);
    };
    const trackIds = response.tracks.map(track => track.id);
    reloadSelectOptionsAndSet(inputs[0], trackIds)

    //inputs[0].setValue(trackIds)
...
function setFormDataFromResponse(url, formMap) {
    ...
    $.get(url, function (response) {
        ...
        for (let [key, value] of formMap) {
            ...
            if (inputDataWrapper) {
                if(value[4]){
                    inputDataWrapper(inputs, response);
                } else {
